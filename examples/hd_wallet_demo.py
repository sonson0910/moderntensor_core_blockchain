...
Demonstrates ModernTensor-like coldkey/hotkey system using HD wallets
...
def demo_basic_usage():
    ...
    console.print(
        Panel.fit(
            ...
            "similar to ModernTensor's coldkey/hotkey system",
    ...
    try:
        ...
        console.print(
            f"[dim]Mnemonic (first 4 words): {' '.join(mnemonic.split()[:4])}...[/dim]"
        )
    ...
    validator_coldkey = wallet_manager.create_coldkey(
        wallet_name, "validator", account_index=0
    )
    ...
    val_hotkey1 = wallet_manager.create_hotkey(
        wallet_name, "validator", "val_operator", address_index=1
    )
    val_hotkey2 = wallet_manager.create_hotkey(
        wallet_name, "validator", "val_backup", address_index=2
    )

    # Create hotkeys for miner
    miner_hotkey1 = wallet_manager.create_hotkey(
        wallet_name, "miner", "miner_worker", address_index=1
    )
    miner_hotkey2 = wallet_manager.create_hotkey(
        wallet_name, "miner", "miner_staking", address_index=2
    )
    ...
    miner_worker_account = wallet_manager.get_account(
        wallet_name, "miner", "miner_worker"
    )
    ...
    miner_worker_private_key = wallet_manager.export_private_key(
        wallet_name, "miner", "miner_worker"
    )
    console.print(f"[green]✅ Miner worker private key exported[/green]")
    console.print(
        f"[dim]Key (first 16 chars): {miner_worker_private_key[:16]}...[/dim]"
    )
...
def demo_wallet_restoration():
    ...
    if wallet_manager.restore_wallet(restored_wallet, demo_mnemonic, password):
        console.print(
            f"[green]✅ Wallet '{restored_wallet}' restored successfully[/green]"
        )
        ...
        if wallet_manager.load_wallet(restored_wallet, password):
            ...
            coldkey = wallet_manager.create_coldkey(
                restored_wallet, "restored_coldkey", 0
            )
            hotkey = wallet_manager.create_hotkey(
                restored_wallet, "restored_coldkey", "restored_hotkey", 1
            )
...
def demo_key_import():
    ...
    try:
        imported_account = wallet_manager.import_account_by_private_key(
            wallet_name, example_private_key, "imported_account"
        )
