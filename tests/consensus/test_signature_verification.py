...
def sample_scores() -> list[ValidatorScore]:
    ...
    return [
        ValidatorScore(task_id="t1", miner_uid="m1_hex", validator_uid="validator_submitter_hex", score=0.95, timestamp=time.time()),
        ValidatorScore(task_id="t2", miner_uid="m2_hex", validator_uid="validator_submitter_hex", score=0.88, timestamp=time.time() + 0.1),
    ]
...
async def test_verify_signature_valid(
    ...
    try:
        ...
        assert verify_immediately is True, "Immediate verification using original key failed!"
        print("DEBUG: Immediate verification successful.")
    except AttributeError as ae:
        # This shouldn't happen if to_primitive exists
        print(f"ERROR: The original 'verification_key' object MISSES to_primitive() method? Details: {ae}")
...
async def test_verify_signature_invalid_signature(
    ...
    tampered_signature_hex = signature_hex[:-1] + ("a" if signature_hex[-1] != "a" else "b")
...
async def test_verify_signature_vkey_mismatch_address(
    ...
    other_vkey_cbor_hex = other_verification_key.to_cbor_hex() # VKey không khớp address
