...
class CoreHDWalletManager:
    ...
    Supports ModernTensor-like coldkey/hotkey system with hierarchical deterministic wallets
    ...
    def create_wallet(
        self, wallet_name: str, password: str, words_count: int = 24
    ) -> str:
        """
    ...
    def load_wallet(self, wallet_name: str, password: str) -> bool:
        ...
        if not os.path.exists(wallet_dir):
            self.console.print(
                f"[bold red]❌ Wallet '{wallet_name}' not found[/bold red]"
            )
            return False
        ...
        try:
            ...
            self.console.print(
                f"[bold green]✅ Wallet '{wallet_name}' loaded successfully[/bold green]"
            )
            return True

        except InvalidToken:
            self.console.print(
                f"[bold red]❌ Invalid password for wallet '{wallet_name}'[/bold red]"
            )
            return False
        except Exception as e:
            self.console.print(
                f"[bold red]❌ Error loading wallet '{wallet_name}': {e}[/bold red]"
            )
            return False

    def create_coldkey(
        self, wallet_name: str, coldkey_name: str, account_index: int = None
    ) -> Dict:
        """
        ...
        try:
            ...
            aptos_account = Account.from_key(private_key_bytes)
            ...
            coldkey_info = {
                ...
                "address": address_hex,
                "public_key": str(aptos_account._key_obj.public_key),
                "created_at": datetime.datetime.now().isoformat(),
                "hotkeys": {},
            }
            ...
            self.console.print(
                f"[bold green]✅ Coldkey '{coldkey_name}' created[/bold green]"
            )
    ...
    def create_hotkey(
        ...
        try:
            ...
            self.console.print(
                f"[bold green]✅ Hotkey '{hotkey_name}' created under coldkey '{coldkey_name}'[/bold green]"
            )
            self.console.print(f"[dim]Address: {address_hex}[/dim]")
    ...
    def get_account(
        self, wallet_name: str, coldkey_name: str, hotkey_name: str = None
    ) -> Account:
        """
    ...
    def list_wallets(self) -> List[str]:
        ...
        if os.path.exists(self.base_dir):
            for item in os.listdir(self.base_dir):
                ...
                if os.path.isdir(wallet_dir) and os.path.exists(
                    os.path.join(wallet_dir, "metadata.json")
                ):
                    wallets.append(item)
        return wallets
    ...
    def display_wallet_info(self, wallet_name: str):
        ...
        if wallet_name not in self.wallets:
            self.console.print(
                f"[bold red]❌ Wallet '{wallet_name}' not loaded[/bold red]"
            )
            return
        ...
        for account_index, account_data in metadata["accounts"].items():
            ...
            hotkey_names = (
                ", ".join(account_data["hotkeys"].keys())
                if hotkey_count > 0
                else "None"
            )
    ...
    def restore_wallet(self, wallet_name: str, mnemonic: str, password: str) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[bold green]✅ Wallet '{wallet_name}' restored successfully[/bold green]"
            )
            return True
    ...
    def import_account_by_private_key(
        self, wallet_name: str, private_key_hex: str, account_name: str
    ) -> Dict:
        """
        ...
        try:
            ...
            self.console.print(
                f"[bold green]✅ Account '{account_name}' imported successfully[/bold green]"
            )
