...
# NOTE: Updated imports for Core blockchain migration
...
main_validator_node_instance: Optional[ValidatorNodeCore] = None
...
async def startup_event():
    ...
    if ValidatorInfo and ValidatorNodeCore and settings:
        try:
            ...
            api_endpoint = (
                settings.get_current_validator_endpoint() or f"http://{host}:{port}"
            )
            ...
            main_validator_node_instance = ValidatorNodeCore(
...
async def run_main_node_loop(node: ValidatorNodeCore):
