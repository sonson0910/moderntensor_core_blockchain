...
async def simulate_strict_validator(validator_id: str, start_delay: int = 0):
    ...
    config = SlotConfig(
        slot_duration_minutes=3.5,  # 3.5 minute slots
        task_assignment_minutes=2,  # 2 minutes for task assignment
        consensus_minutes=1,  # 1 minute for consensus
        metagraph_update_seconds=30,  # 30 seconds for metagraph
    )

    coordinator = SlotCoordinator(
        validator_uid=validator_id, slot_config=config, enable_strict_enforcement=True
    )
    ...
    class MockConsensusModule:
        ...
        async def submit_consensus_to_blockchain(self, slot, consensus):
            print(
                f"⛓️ {self.validator_id} P2P: Submitting to blockchain for slot {slot}"
            )
    ...
    print(
        f"📊 {validator_id} Status: {status['current_phase']} phase, {status['phase_remaining_seconds']}s remaining"
    )
