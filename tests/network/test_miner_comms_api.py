...
def mock_validator_node() -> MagicMock:
    ...
    mock_node.add_miner_result = AsyncMock(
        return_value=True
...
def test_submit_result_success(test_client: TestClient, mock_validator_node: MagicMock):
    ...
    assert (
        ...
        == f"Result for task {payload_dict['result_id']} accepted."
    )
    ...
    assert (
        received.task_id == payload_dict["result_id"]
    )  # Endpoint uses result_id for task_id
    assert (
        received.miner_uid == payload_dict["miner_id"]
    )  # Endpoint uses miner_id for miner_uid
...
def test_submit_result_invalid_payload(
...
def test_submit_result_processing_error(
    test_client: TestClient, mock_validator_node: MagicMock
):
    ...
    mock_validator_node.add_miner_result.side_effect = Exception(
        "Lỗi xử lý kết quả nội bộ"
    )
