...
contract ModernTensor is ReentrancyGuard, Ownable {
    ...
    // Enums
    enum NodeStatus { INACTIVE, ACTIVE, SUSPENDED, BANNED }
    ...
    function registerMiner(
        ...
        require(miners[msg.sender].owner == address(0), "Miner already registered");
        ...
        subnets[subnetUid].totalStake = subnets[subnetUid].totalStake.add(stakeAmount);
        
        emit MinerRegistered(msg.sender, uid, subnetUid, stakeAmount, apiEndpoint);
    }
    ...
    function registerValidator(
        ...
        emit ValidatorRegistered(msg.sender, uid, subnetUid, stakeAmount, apiEndpoint);
    }
    ...
    function stakeBitcoin(
        ...
        bitcoinStakes[msg.sender].push(BitcoinStakeInfo({
            txHash: txHash,
            amount: amount,
            lockTime: lockTime,
            unlockTime: 0,
        ...
        totalBitcoinStake[msg.sender] = totalBitcoinStake[msg.sender].add(amount);
        ...
        if (miners[msg.sender].owner == msg.sender) {
            miners[msg.sender].bitcoinStake = miners[msg.sender].bitcoinStake.add(amount);
        } else {
            validators[msg.sender].bitcoinStake = validators[msg.sender].bitcoinStake.add(amount);
        }
    ...
    function updateMinerScores(
        ...
        require(newPerformance <= MAX_PERFORMANCE_SCORE, "Performance score too high");
    ...
    function updateValidatorScores(
        ...
        require(validators[validatorAddr].owner != address(0), "Validator not found");
        require(newAccuracy <= MAX_PERFORMANCE_SCORE, "Accuracy score too high");
    ...
    function getStakingMultiplier(uint256 tier) external pure returns (uint256) {
    ...
    function getSubnetInfo(uint64 subnetId) external view returns (SubnetInfo memory) {
    ...
    function getBitcoinStakes(address user) external view returns (BitcoinStakeInfo[] memory) {
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        require(coreToken.transfer(owner(), balance), "Emergency withdraw failed");
    }
