...
from ..consensus.bittensor_consensus import BittensorConsensus
...
class TestBitcoinStakingIntegration:
    ...
    async def setup_test_environment(self):
        ...
        test_accounts = {
            ...
            'deployer': Account.from_key("a07b6e0db803f9a21ffd1001c76b0aa0b313aaba8faab8c771af47301c4452b4")
        }
    ...
    async def test_complete_dual_staking_flow(self, setup_test_environment):
        ...
        miner1_uid = await self._test_miner_registration(client, env['accounts']['miner1'])
        miner2_uid = await self._test_miner_registration(client, env['accounts']['miner2'])
        ...
        validator1_uid = await self._test_validator_registration(client, env['accounts']['validator1'])
        validator2_uid = await self._test_validator_registration(client, env['accounts']['validator2'])
        ...
        await self._test_bitcoin_staking(client, env['accounts']['validator1'], validator1_uid)
        print("✅ Bitcoin staking completed")
        
        # 4. Test Staking Tier Calculation
        print("4️⃣ Testing Staking Tier Calculation...")
        tier1 = await self._test_staking_tier_calculation(client, env['accounts']['miner1'].address)
        tier2 = await self._test_staking_tier_calculation(client, env['accounts']['validator1'].address)
        ...
        print(f"✅ Consensus round completed with {len(consensus_round.rewards)} rewards")
        ...
        print(f"✅ Network metrics: {metrics.total_miners} miners, {metrics.total_validators} validators")
        
        print("🎉 Complete Dual Staking Flow Test PASSED!")
        return True
    
    async def _test_miner_registration(self, client: ModernTensorCoreClient, account: Account) -> int:
    ...
    async def _test_validator_registration(self, client: ModernTensorCoreClient, account: Account) -> int:
    ...
    async def _test_bitcoin_staking(self, client: ModernTensorCoreClient, account: Account, uid: int):
        """Test Bitcoin staking functionality"""
        try:
            # Simulate Bitcoin transaction
            bitcoin_tx_hash = Web3.keccak(text=f"btc_tx_{account.address}_{time.time()}")
    ...
    async def _test_staking_tier_calculation(self, client: ModernTensorCoreClient, address: str) -> int:
    ...
    async def _test_performance_updates(self, client: ModernTensorCoreClient, miner1_uid: int, miner2_uid: int):
        ...
        try:
            ...
            client.update_miner_scores(
                str(miner1_uid),
                new_performance=performance1,
                new_trust_score=trust1
            )
            
            # Update miner 2
            client.update_miner_scores(
                str(miner2_uid),
                new_performance=performance2,
                new_trust_score=trust2
            )
            
            print(f"  📊 Updated performances: Miner1={performance1/1000000:.3f}, Miner2={performance2/1000000:.3f}")
            return True
...
class TestStakingTiers:
    ...
    @pytest.mark.parametrize("core_stake,btc_stake,expected_tier", [
    ...
    def test_staking_tier_logic(self, core_stake: int, btc_stake: int, expected_tier: int):
    ...
    consensus = ModernConsensus(None, None)
        ...
        assert multiplier == expected_multiplier, f"Expected {expected_multiplier}, got {multiplier}"
...
class TestConsensusAlgorithms:
    ...
    def test_consensus_weight_calculation(self):
        ...
        validators = {
            ...
            3: {'stake': 2000 * 10**18, 'bitcoin_stake': 2 * 10**8, 'trust_score': 0.95}
        }
    ...
    from ..consensus.modern_consensus import ModernConsensus
        ...
        miners = {
            ...
            3: {'stake': 1500 * 10**18, 'bitcoin_stake': 1.5 * 10**8, 'trust_score': 0.95}
        }
