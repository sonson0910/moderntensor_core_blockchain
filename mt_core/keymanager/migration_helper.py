...
class MigrationHelper:
    ...
    def __init__(self, base_dir: str = None):
        ...
        self.hd_wallet_manager = CoreHDWalletManager(self.base_dir)
        self.utils = WalletUtils(self.base_dir)

    def create_migration_wallet(
        self, wallet_name: str, password: str, word_count: int = 24
    ) -> str:
        """
        ...
        try:
            mnemonic = self.hd_wallet_manager.create_wallet(
                wallet_name, password, word_count
            )
    ...
    def migrate_from_private_keys(
        self,
        private_keys: Dict[str, str],
        wallet_name: str,
        password: str,
        coldkey_name: str = "migrated_coldkey",
    ) -> Dict[str, str]:
        """
        ...
        for account_name, private_key_hex in private_keys.items():
            try:
                ...
                test_account = Account.from_key(bytes.fromhex(private_key_hex))
                original_address = test_account.address

                # Create hotkey with the same name
                hotkey_info = self.hd_wallet_manager.create_hotkey(
                    wallet_name, coldkey_name, account_name
                )
    ...
    def migrate_from_old_keymanager(
        self, old_keymanager_dir: str, wallet_name: str, password: str
    ) -> Dict[str, str]:
        """
        ...
        if not old_dir.exists():
            console.print(
                f"[red]❌ Old keymanager directory not found: {old_keymanager_dir}[/red]"
            )
            return {}
        ...
        if hotkeys_file.exists():
            try:
                ...
                console.print(
                    f"[yellow]Found {len(hotkeys_data)} accounts in hotkeys.json[/yellow]"
                )

                for coldkey_name, coldkey_data in hotkeys_data.items():
                    # Create coldkey
                    if not self.hd_wallet_manager.coldkey_exists(
                    ...
                    if "hotkeys" in coldkey_data:
                        for hotkey_name, hotkey_data in coldkey_data["hotkeys"].items():
                            try:
                                ...
                                console.print(
                                    f"[green]✅ Migrated {coldkey_name}.{hotkey_name}[/green]"
                                )

                            except Exception as e:
                                console.print(
                                    f"[red]❌ Failed to migrate {coldkey_name}.{hotkey_name}: {e}[/red]"
                                )
    ...
    def generate_migration_config(
        self,
        migration_results: Dict[str, str],
        wallet_name: str,
        coldkey_name: str = "migrated_coldkey",
    ) -> str:
        """
...
"""
    ...
    def create_migration_summary(
        self, migration_results: Dict[str, str], wallet_name: str
    ) -> None:
        """
        ...
        for account_name, info in migration_results.items():
            if isinstance(info, dict):
                ...
                table.add_row(
                    account_name,
                    (
                        f"{old_addr[:10]}...{old_addr[-6:]}"
                        if len(old_addr) > 16
                        else old_addr
                    ),
                    (
                        f"{new_addr[:10]}...{new_addr[-6:]}"
                        if len(new_addr) > 16
                        else new_addr
                    ),
...
"""
    ...
    def _migrate_private_keys_wizard(self) -> None:
        ...
        try:
            ...
            templates = self.generate_code_migration_templates(
                migration_results, wallet_name, coldkey_name
            )

            # Save templates
            template_file = (
                Path(self.base_dir) / f"{wallet_name}_migration_templates.py"
            )
            ...
            console.print(
                f"[green]✅ Migration templates saved: {template_file}[/green]"
            )
    ...
    def _migrate_old_keymanager_wizard(self) -> None:
        ...
        # Perform migration
        try:
            migration_results = self.migrate_from_old_keymanager(
                old_dir, wallet_name, password
            )
...
"""
        ...
        console.print(
            Panel(guidance, title="Migration Guidance", border_style="yellow")
        )
