...
class ModernTensorClient:
    ...
    async def get_miner_info(self, miner_address: str) -> MinerInfo:
        ...
        Returns:
            MinerInfo: Thông tin chi tiết của miner.
        """
        logger.info(f"Querying miner info for {miner_address}")
        
        # Gọi view function để lấy thông tin miner
        # Trong một triển khai thực tế, bạn sẽ cần định nghĩa các view function trong contract
        # Giả định hàm này trả về các giá trị cần thiết
        try:
            ...
            subnet_uid = result[1]
            stake = result[2]
            last_performance = result[3] / 1_000_000  # Unscale
            trust_score = result[4] / 1_000_000  # Unscale
            status = result[5]
            api_endpoint = result[6]
            registration_timestamp = result[7]
            ...
            return MinerInfo(
                ...
                weight=trust_score,  # For simplicity
                stake=stake,
                status=status,
            ...
            logger.error(f"Error querying miner info: {e}")
            # Trả về một đối tượng rỗng nếu không tìm thấy
            return MinerInfo(
                uid="0x0",
                address=miner_address,
