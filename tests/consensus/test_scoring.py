...
from unittest.mock import MagicMock, call, AsyncMock # Keep MagicMock if needed elsewhere, add AsyncMock if patching async
...
def sample_tasks_sent(validator_uid: str) -> Dict[str, TaskAssignment]:
    ...
    ts = time.time()
    task1 = TaskAssignment(task_id="task_s01", miner_uid="miner_S_A_hex", task_data={"input": 1}, validator_uid=validator_uid, timestamp_sent=ts, expected_result_format=dict)
    time.sleep(0.01); ts = time.time()
    task2 = TaskAssignment(task_id="task_s02", miner_uid="miner_S_B_hex", task_data={"input": 2}, validator_uid=validator_uid, timestamp_sent=ts, expected_result_format=dict)
    time.sleep(0.01); ts = time.time()
    task3 = TaskAssignment(task_id="task_s03", miner_uid="miner_S_A_hex", task_data={"input": 3}, validator_uid=validator_uid, timestamp_sent=ts, expected_result_format=dict)
    return { task1.task_id: task1, task2.task_id: task2, task3.task_id: task3 }

# --- UPDATED Fixture ---
@pytest.fixture
def sample_results_received_dict(sample_tasks_sent: Dict[str, TaskAssignment]) -> Dict[str, List[MinerResult]]:
    """Tạo dict các kết quả nhận được mẫu, nhóm theo task_id."""
    task1 = sample_tasks_sent["task_s01"]
    task2 = sample_tasks_sent["task_s02"]
    res1 = MinerResult(task_id=task1.task_id, miner_uid=task1.miner_uid, result_data={"output": 10}, timestamp_received=time.time() + 0.1)
    time.sleep(0.01)
    res2 = MinerResult(task_id=task2.task_id, miner_uid=task2.miner_uid, result_data={"output": 20}, timestamp_received=time.time() + 0.1)
    res_unmatched = MinerResult(task_id="task_s99", miner_uid="miner_S_C_hex", result_data={"output": 99}, timestamp_received=time.time() + 0.2) # This won't be in the dict keys usually
    res1_duplicate = MinerResult(task_id=task1.task_id, miner_uid=task1.miner_uid, result_data={"output": 11}, timestamp_received=time.time() + 0.3)
    # Result from wrong miner for task 2
    res2_wrong_miner = MinerResult(task_id=task2.task_id, miner_uid="miner_WRONG_hex", result_data={"output": 22}, timestamp_received=time.time() + 0.4)

    results_dict = {}
    # Group by task_id
    results_dict[res1.task_id] = [res1, res1_duplicate] # task_s01 has two results
    results_dict[res2.task_id] = [res2, res2_wrong_miner] # task_s02 has two results (one wrong miner)
...
def test_score_results_logic_basic(
    ...
    def score_side_effect(task_data, result_data):
        ...
        if task_data == sample_tasks_sent["task_s01"].task_data: return expected_score1
        if task_data == sample_tasks_sent["task_s02"].task_data: return expected_score2
        return 0.5 # Default
...
def test_score_results_logic_no_results(
    ...
    mock_calculate_score = mocker.patch("sdk.consensus.scoring._calculate_score_from_result")
...
def test_score_results_logic_no_matching_task(
    ...
    results_only_unmatched = {
        "task_s99": [MinerResult(task_id="task_s99", miner_uid="miner_S_C_hex", result_data={}, timestamp_received=time.time())]
    }
...
def test_score_results_logic_duplicate_and_wrong_miner(
    ...
    mock_calculate_score = mocker.patch("sdk.consensus.scoring._calculate_score_from_result", return_value=mock_score)
